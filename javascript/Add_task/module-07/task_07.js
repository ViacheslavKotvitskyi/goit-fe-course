...
/* result.push(arr.filter(item => item > num));
  return result;
}; */
...
/* result.push(arr.map(item => item * num));
  return result;
}; */
...
//    accumulator += args[i];
//  }
/* accumulator = args.reduce((accumulator, item) => (accumulator += item), 0);
...
/* const getTotal = order => {
  return Object.values(order).reduce((total, item) => (total += item), 0);
};
...
const getTotal = (products, order) =>
  Object.entries(order).reduce(
    (total, item) => total + products[`${item[0]}`] * item[1],
    0,
  );
